...
if file is not None:
    ...
    regime = pd.read_excel(file, sheet_name="regime", parse_dates=["Date"], index_col=0, header=0).dropna()
    ...
    with st.form("Resampling Parameters", clear_on_submit=False):
        ...
        col20, col21, col22, col23,col24 = st.columns([1, 1, 1, 1,2])
        ...
        with col23:
            ...
            regime_option = st.selectbox(
                    'Regime', regime['regime'].drop_duplicates().tolist())

            regime_index = regime[regime['regime']==regime_option].index
            # st.write(input_price[regime_index.is_month_end])
            ...
            if data_freq=='Monthly':
                ...
                annualization = 12
                freq = "monthly"
                regime_index = regime_index[regime_index.is_month_end==True]
        ...
        if summit and (('EF' not in st.session_state) or ([st.session_state.nPort, st.session_state.nSim,
            ...
            ##############################################################################################################
            # st.session_state.regime_index = regime_index
            ...
            # #st.session_state.input_ret = st.session_state.input_ret.loc[st.session_state.regime_index]
            #
            # valid_index = st.session_state.regime_index.intersection(st.session_state.input_ret.index)
            # st.session_state.input_ret = st.session_state.input_ret.loc[valid_index]
            #
            # st.session_state.EF = Regime_resampled_mvo.simulation(st.session_state.input_ret,
            #                                                st.session_state.nSim, st.session_state.nPort,
            #                                                st.session_state.input_universe,
            #                                                st.session_state.constraint_range,
            #                                                annualization)
            ##############################################################################################################
            # 새로운 리스트를 초기화하여 각 regime_index에 대한 EF 결과를 저장합니다.
            st.session_state.all_EF = []

            # 여러 regime_index 값을 포함하는 리스트를 정의합니다.
            regime_indices = regime['regime'].drop_duplicates().tolist()

            for regime_index in regime_indices:
                # regime_index가 문자열일 수 있으므로, 이를 `input_ret`의 인덱스와 일치시키기 위한 작업 필요
                # 예를 들어 regime_index를 특정 날짜의 리스트로 변환하거나, 아니면 범위로 정의된 경우 해당 범위를 적용
                st.session_state.regime_index = regime[regime['regime'] == regime_index].index
                ...
                st.session_state.EF = Regime_resampled_mvo.simulation(st.session_state.input_ret,
                                                     ...
                                                     st.session_state.constraint_range,
                                                     annualization)
                # regime_index 변수를 새로운 열로 추가
                st.session_state.EF['regime_index'] = regime_index
                # 각 regime_index에 대한 결과를 리스트에 추가
                st.session_state.all_EF.append(st.session_state.EF)

            st.session_state.EF = pd.concat(st.session_state.all_EF, ignore_index=True)
            st.session_state.EF_tmp = st.session_state.EF.copy()
            st.session_state.EF = st.session_state.EF.drop(columns=['regime_index'])

            # 최종적으로 모든 EF 결과가 st.session_state.all_EF에 저장됩니다.
            ##############################################################################################################
            ...
            #Result = pd.concat([A.drop(['symbol'], axis=1).T, st.session_state.EF], axis=0, join='outer')
    ...
    if 'EF' in st.session_state:
        ...
        st.download_button(
           ...
           # data=st.session_state.Result.to_csv(index=False),
            data=st.session_state.EF_tmp.to_csv(index=False, encoding='ISO-8859-1'),
            mime='text/csv',
